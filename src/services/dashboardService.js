...
export class DashboardService {
  ...
  static async getCovidDataByPeriod(period = '30d', limit = 1000) {
    try {
      console.log(`Récupération données ${period}`)

      // Utiliser fetch directement
      const response = await fetch(`http://localhost:5000/api/data?page_size=${limit}&page=1`)
      const data = await response.json()
      
      console.log(`✅ ${data.length} éléments récupérés`)
      
      if (!Array.isArray(data) || data.length === 0) {
        return this.generateTestDataFromCollections()
      }

      // Désactiver temporairement le filtrage pour tester
